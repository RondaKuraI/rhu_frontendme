    ...
    <v-container>
                ...
                <v-card-text>       
                  ...
                  <v-row>
                    <v-col cols="12" sm="6" md="12">
                      <v-text-field v-model="editedItem.reference_id" label="National Drug Code" required variant="solo-filled" density="comfortable"></v-text-field> 
                      <v-text-field v-model="editedItem.date" label="Medicine Name" required variant="solo-filled" density="comfortable"></v-text-field>
                      <v-text-field v-model="editedItem.time" label="Medicine Name" required variant="solo-filled" density="comfortable"></v-text-field>
                      <v-select v-model="editedItem.reason" :items="med_types" density="comfortable" label="Medicine Type" variant="solo-filled"></v-select>
                      <v-text-field v-model="editedItem.status" label="Stocks" required variant="solo-filled" density="comfortable" ></v-text-field>
                    </v-col>
                  </v-row>
                  </v-container>
                </v-card-text>
        ...
        <v-card elevation="10" class="mt-2">        
          <v-data-table
            :headers="headers"
            :items="desserts"
            :sort-by="[{ key: 'reference_id', order: 'asc' }]"
            >
            <template v-slot:top>
...
import Admin_NavBar from '@/components/Admin/Admin_NavBar.vue';
  export default {
      ...
      data: () => ({
        ...
        loading: false,
        headers: [
          { title: 'ID', key: 'reference_id',  },
          { title: 'Schedule Date', key: 'date', sortable: false, },
          { title: 'Time', key: 'time', },
      ...
      computed: {
        formTitle () {
          return this.editedIndex === -1 ? 'Add Appointment' : 'Edit Item'
        }
      },
      ...
      methods: {
        ...
        async save () {
          if (this.editedIndex > -1) {
            // edit
            const response = await axios.put(`api/updateAppointment/${this.editedItem.id}`, this.editedItem);
            console.log(response.data);
            Object.assign(this.desserts[this.editedIndex], this.editedItem)
          } else {
            
            const response = await axios.post('api/newappointment', this.editedItem);
            console.log(response.data);
